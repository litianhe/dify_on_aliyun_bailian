...
class AliYunLargeLanguageModel(LargeLanguageModel):
    ...
    def validate_credentials(self, model: str, credentials: dict) -> None:
        ...
        try:
            ...
            bailian_api_key = credentials["bailian_api_key"]
            bailian_app_id = credentials["bailian_app_id"]
            if bailian_api_key and bailian_app_id:
                r = self.send_input_message(bailian_app_id, bailian_api_key, "hello", stream=False)
    ...
    def send_input_message(self, bailian_app_id: str, bailian_api_key: str, prompt_message:str, stream:bool = True) -> Response:
        url = f"https://dashscope.aliyuncs.com/api/v1/apps/{bailian_app_id}/completion"
        headers = self._prepare_headers(bailian_api_key, stream=stream)
        body = {
            "input": {
    ...
    def _prepare_headers(self, bailian_api_key, stream):
        h = {"Content-Type": "application/json", "Authorization": f"Bearer {bailian_api_key}"}
        if stream:
    ...
    def _handle_error(self, response):
        if response.status_code in {402, 409, 500}:
            ...
            if response.text:
                error_message = json.loads(response.text).get("message", "Unknown error occurred")
                raise Exception(f"Failed. Status code: {response.status_code}. Error: {error_message}")
    ...
    def _generate(
        ...
        credentials_kwargs = {"bailian_api_key": credentials["bailian_api_key"], "bailian_app_id": credentials["bailian_app_id"]}
